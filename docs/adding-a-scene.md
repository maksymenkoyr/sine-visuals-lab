...
2. Each entry in `settings: SceneSetting[]` (spec'd in `sceneSettings.ts`) becomes
   ...
   are what the user sees; `type: "boolean"` renders a checkbox instead. Mark
   one `draft: true` while you're still deciding whether it deserves to ship —
   a draft is scrubbable in a tuning session and invisible to a user (see
   `docs/tuning.md`'s promote → scrub → keep/draft/bake loop).
