...
**Naming convention:** modules below are named by basename, under `src/` unless
written out — `analyser.ts` means `src/audio/analyser.ts`. Fully qualified only
where a basename collides: `src/net/room.ts` vs `server/room.ts`,
`src/audio/capture.ts` vs `src/tuning/capture.ts` (the tuning contact-sheet
capture — see `docs/tuning.md`), and `src/render/scenes/index.ts`.

## Mic to pixel, on one device

```mermaid
flowchart LR
  mic(["mic / display audio"]) --> capture["audio/capture.ts"]
  capture --> analyser["analyser.ts\nFFT + band split"]
  analyser --> features["features.ts\nadaptive floor/peak AGC"]
  features -->|FeatureFrame| sensitivity["sensitivity.ts\nSensitivity / Contrast / Smoothing"]
  sensitivity --> animClock["animClock.ts"]
...
**Capture and normalize.** `src/audio/capture.ts` opens the mic (or, on a host,
display audio). `analyser.ts` runs the FFT and splits it into bands (`bandSplit.ts` /
...
`sensitivity.ts` applies the user's Sensitivity/Contrast(Acceleration)/Smoothing
...
beat/bar clock, per-band energy and onset pulses, section intensity. **Scenes never
read `FeatureFrame` fields directly for anything animated; they read `AnimFrame`**,
...
`render/scenes/` implements. Most scenes are built via `fullscreenScene.ts`'s
...
**Routing.** `router.ts` decides which scene mounts (`#/` → gallery, `#/v/<sceneId>`
→ one scene), read by `app.ts`, the phone/controller/gallery entry (`index.html`).
...
```mermaid
flowchart LR
  host(["host device"]) --> protocol["protocol.ts\nencode FeatureFrame"]
  protocol --> room["Room (server/room.ts)\nrelays bytes, never parses"]
  room --> netRoom["net/room.ts\nrender-delay + jitter/slew"]
  netRoom --> clock["clock.ts\nClockSync"]
...
A device that's alone in a room (no pairing) never touches any of this — `app.ts`
drives `AnimFrame` straight from its own local `FeatureFrame`s.

## Where the quality/perf ceiling comes from

`tier.ts` (`detectTier`) picks a quality tier once at startup; `governor.ts` (the
quality governor) can step it down at runtime under sustained frame-time pressure.
A scene's `minTier` (on the `Scene` interface) opts it out of running below a given
tier at all.
