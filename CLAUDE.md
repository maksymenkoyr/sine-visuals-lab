...
| The tempo tracker (BPM in `src/audio/features.ts`, `src/audio/tempoAnalyzer.ts`, `src/render/beatClock.ts`) | `tests/tempoEval.test.ts` header — run `npm run eval:tempo` before and after; it scores the real tracker on synthesized tracks with known beats, both the render-tick and fixed-hop paths |
