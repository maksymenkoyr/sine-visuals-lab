...
//
// Audio wiring: `energy` and `lowPulse`/`beatPulse` drive the centre
// emitter's push and its dye output (see emitterState below); `flowPhase`
// times both the emitter's slow sway and the periodic secondary splats
// (splatEnvelope); the Swirl/Dye fade settings are themselves music-reactive
// via `auto` weights, same as every other scene's SETTINGS. Only the display
// program goes through uploadCommonUniforms — the sim's own programs
...
export const EMIT_SIGMA = 0.11;
export const EMIT_FORCE_BASE = 110;
...
export const DYE_BASE = 0.22;
export const DYE_ENERGY = 1.5;
 ...
 *  along the mirror seam rotated by a slow sway), slots 1..SPLAT_SLOTS-1 are
 *  periodic secondary splats (tag 0) from SPLAT_POINTS. */
export function emitterState(inp: EmitterInputs, out: Splat[]): Splat[] {
  ...
  const dye = inp.dyeFlow * (DYE_BASE + DYE_ENERGY * inp.energy);

  const slot0 = out[0] ?? (out[0] = { x: 0, y: 0, sigma: 0, fx: 0, fy: 0, dye: 0, tag: 0 });
  slot0.x = ex;
  slot0.y = ey;
  slot0.sigma = EMIT_SIGMA;
  ...
  for (let i = 1; i < SPLAT_SLOTS; i++) {
    ...
    const env = splatEnvelope(inp.flowPhase, i - 1);
    const slot = out[i] ?? (out[i] = { x: 0, y: 0, sigma: 0, fx: 0, fy: 0, dye: 0, tag: 0 });
    slot.x = px;
    slot.y = py;
...
export const SETTINGS: SceneSetting[] = [
  {
    key: "emitStrength",
    label: "Emitter push",
    description: "How hard the centre emitter drives the fluid",
    group: "Flow",
    min: 0,
    max: 1,
    step: 0.05,
  ...
  },
  {
    key: "beatKick",
    label: "Beat kick",
    description: "Extra emitter push on bass hits and beats",
    group: "Flow",
    min: 0,
    max: 1,
    step: 0.05,
    ...
    reads: ["anim.lowOnset", "feature.onset"],
  },
  {
    key: "curl",
    label: "Swirl",
...
void main() {
  ...
  vec3 c = hueRotate(regionColour(uv.x), uHueShift * 6.28318);
  c = mix(c, palette(0.2 + 0.6 * uv.x + uHueShift, uPalA, uPalB, uPalC, uPalD), ${PALETTE_BLEND.toFixed(2)});
  c = mix(c, NEON_PURPLE, ${PURPLE_MIX.toFixed(2)} * clamp(dye.g / max(dye.r, 1e-4), 0.0, 1.0));

  float sparkle = 1.0 + uSparkle * (1.2 * uHigh + 1.5 * uHighPulse);
  vec3 col = BG + c * edge * ${LINE_GAIN.toFixed(2)} * sparkle + c * glow * uEdgeGlow * ${GLOW_GAIN.toFixed(2)} + c * dye.r * ${FILL_GAIN.toFixed(2)};
...
function createFluidScene(): Scene {
  ...
  return {
    ...
    render(ctx, frame, viewport, palette, anim) {
      ...
      sim.step({ dt, curl, dissipation, energy: frame.energy, splats });
