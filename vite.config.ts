...
export default defineConfig(({ command }) => ({
  ...
  // suspenders. legalNoticesPlugin is the exception: it ships LICENSE.txt /
  // THIRD-PARTY-NOTICES.txt / PRIVACY.txt with both the dev server and the
  // build, so it's registered unconditionally — see its header.
  plugins:
    command === "serve"
      ? [basicSsl(), tuningPlugin(), sceneLinksPlugin(), legalNoticesPlugin()]
      : [legalNoticesPlugin()],
  build: {
