...
import type { QualitySettings } from "../src/render/quality.ts";

function baseline(): QualitySettings {
  return { preset: "mid", renderScale: 0.75, maxParticles: 50_000, raymarchSteps: 64, bloomPasses: 2, detail: 0.7 };
}
...
describe("createQualityGovernor", () => {
  it("does nothing on comfortable frame times", () => {
    ...
    expect(quality.renderScale).toBeCloseTo(0.75, 6);
    expect(quality.raymarchSteps).toBe(64);
    expect(quality.detail).toBeCloseTo(0.7, 6);
    expect(gov.fraction).toBeCloseTo(1, 6);
  });
  ...
  it("steps quality down under sustained slow frames a cut actually fixes", () => {
    ...
    expect(quality.renderScale).toBeLessThan(0.75);
    expect(quality.raymarchSteps).toBeLessThan(64);
    expect(quality.detail).toBeLessThan(0.7);
    expect(gov.fraction).toBeLessThan(1);
  });

  it("never mutates the preset label, only numeric knobs", () => {
    const quality = baseline();
    const gov = createQualityGovernor(quality, TARGET_MS);
    let t = 0;
    for (let i = 0; i < 500; i++) {
    ...
    expect(quality.preset).toBe("mid");
  });

  it("recovers after slow frames are followed by a long comfortable stretch, and fraction tracks level back up", () => {
    const quality = baseline();
    const gov = createQualityGovernor(quality, TARGET_MS);
    let t = 0;
    ...
    const droppedFraction = gov.fraction;
    expect(droppedFraction).toBeLessThan(1);
    ...
    expect(gov.fraction).toBeGreaterThan(droppedFraction);
  });
  ...
  it("floors out rather than driving settings to zero/negative under extreme sustained load", () => {
    ...
    expect(quality.renderScale).toBeGreaterThan(0);
    expect(quality.raymarchSteps).toBeGreaterThan(0);
    expect(quality.detail).toBeGreaterThan(0);
  });
